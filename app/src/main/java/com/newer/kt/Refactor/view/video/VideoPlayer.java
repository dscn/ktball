 ...
 * On 2016/07/24
 */
public abstract class VideoPlayer extends FrameLayout implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, VideoMediaManager.JCMediaPlayerListener, TextureView.SurfaceTextureListener {
    ...
    public VideoResizeTextureView textureView;
    ...
    protected static VideoBuriedPoint JC_BURIED_POINT;
    ...
    protected static void setJcBuriedPoint(VideoBuriedPoint jcBuriedPoint) {
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.start) {
            ...
            if (mCurrentState == CURRENT_STATE_NORMAL || mCurrentState == CURRENT_STATE_ERROR) {
                if (!mUrl.startsWith("file") && !VideoUtils.isWifiConnected(getContext()) && !WIFI_TIP_DIALOG_SHOWED) {
                    showWifiDialog();
                    return;
                }
                startButtonLogic();
                ...
                VideoMediaManager.instance().mediaPlayer.pause();
                ...
                VideoMediaManager.instance().mediaPlayer.start();
            ...
            if (mIfCurrentIsFullscreen) {
                ...
                VideoMediaManager.instance().setLastListener(this);
                VideoMediaManager.instance().setListener(null);
                IF_FULLSCREEN_FROM_NORMAL = true;
                IF_RELEASE_WHEN_ON_PAUSE = false;
                VideoFullScreenActivity.startActivityFromNormal(getContext(), mCurrentState, mUrl, VideoPlayer.this.getClass(), this.mObjects);
            }
    ...
    protected void prepareVideo() {
        ...
        VideoMediaManager.instance().prepare(mUrl, mMapHeadData, mLooping);
        ...
        public void onAudioFocusChange(int focusChange) {
            switch (focusChange) {
                ...
                case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                    if (VideoMediaManager.instance().mediaPlayer.isPlaying()) {
                        VideoMediaManager.instance().mediaPlayer.pause();
                    }
                    break;
    ...
    protected void addTextureView() {
        ...
        textureView = null;
        textureView = new VideoResizeTextureView(getContext());
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        ...
        VideoMediaManager.instance().setDisplay(mSurface);
    }
        ...
        if (id == R.id.surface_container) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    dismissVolumDialog();
                    if (mChangePosition) {
                        VideoMediaManager.instance().mediaPlayer.seekTo(mSeekTimePosition);
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        ...
        if (fromUser) {
            ...
            VideoMediaManager.instance().mediaPlayer.seekTo(time);
    ...
    public void onPrepared() {
        ...
        if (seekToInAdvance != -1) {
            VideoMediaManager.instance().mediaPlayer.seekTo(seekToInAdvance);
            seekToInAdvance = -1;
        }
        startProgressTimer();
        setStateAndUi(CURRENT_STATE_PLAYING);
    ...
    public void onAutoCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {//如果在进入全屏后播放完就初始化自己非全屏的控件
            ...
            VideoMediaManager.instance().lastListener().onAutoCompletion();
        }
    ...
    public void onCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {//如果在进入全屏后播放完就初始化自己非全屏的控件
            IF_FULLSCREEN_FROM_NORMAL = false;
            VideoMediaManager.instance().lastListener().onCompletion();
        }
        VideoMediaManager.instance().setListener(null);
        VideoMediaManager.instance().setLastListener(null);
        VideoMediaManager.instance().currentVideoWidth = 0;
        VideoMediaManager.instance().currentVideoHeight = 0;
    ...
    public void onVideoSizeChanged() {
        int mVideoWidth = VideoMediaManager.instance().currentVideoWidth;
        int mVideoHeight = VideoMediaManager.instance().currentVideoHeight;
    ...
    public void onBackFullscreen() {
        mCurrentState = VideoMediaManager.instance().lastState;
    ...
    protected int getCurrentPositionWhenPlaying() {
        ...
        if (mCurrentState == CURRENT_STATE_PLAYING || mCurrentState == CURRENT_STATE_PAUSE) {
            try {
                position = (int) VideoMediaManager.instance().mediaPlayer.getCurrentPosition();
    ...
    protected int getDuration() {
        int duration = 0;
        try {
            duration = (int) VideoMediaManager.instance().mediaPlayer.getDuration();
        } catch (IllegalStateException e) {
            e.printStackTrace();
            return duration;
        }
    ...
    protected void quitFullScreenGoToNormal() {
        ...
        VideoMediaManager.instance().setDisplay(null);
        VideoMediaManager.instance().setListener(VideoMediaManager.instance().lastListener());
        VideoMediaManager.instance().setLastListener(null);
        VideoMediaManager.instance().lastState = mCurrentState;//save state
    ...
    public void backFullscreen() {
        ...
        if (mIfFullscreenIsDirectly) {
            VideoMediaManager.instance().mediaPlayer.release();
            finishFullscreenActivity();
        } else {
    ...
    protected boolean isCurrentMediaListener() {
        return VideoMediaManager.instance().listener() != null
                && VideoMediaManager.instance().listener() == this;
    }
}
